...
export default function Step4DateTimeCard({
  ...
  const isLocationValid = useMemo(
    () => !isHomeCare || location.trim().length > 0,
    [isHomeCare, location]
  );

  // Submit guard: امنع الإرسال إذا HomeCare والموقع فاضي
  const onSubmitClick = () => {
    if (isHomeCare && !location.trim()) {
      setLocError("الموقع مطلوب في حالة الزيارات المنزلية.");
      return;
    }
    handleSubmit();
  };
  ...
  }, [dispatch, isHomeCare, selectedDate, selectedDoctor, setSelectedSlot]);

  const { times, homeCareTimes, status1 } = useSelector(
    ...
    <>
      ...
      <div className="flex justify-between items-center ">
        ...
        <button
          ...
          disabled={status1 === "loading" || !isLocationValid}
          className={`px-4 py-2 rounded-2xl text-white hover:bg-[#7f1551] ${
            status1 === "loading" || !isLocationValid
              ? "bg-[#972F6AFF]/60 cursor-not-allowed"
              : "bg-[#972F6AFF]"
          }`}
        >
          Submit
              ...
              homeCareTimes.map((s, i) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-md py-2 my-28 text-[17px] mx-4 text-center transition ${
                      isSelected
      ...
      <div className="flex justify-between items-start gap-4 ">
        {isHomeCare ? (
          // HomeCare: location REQUIRED
          <div className="w-full max-w-xs">
            <div
              className={`flex items-center rounded-2xl px-3 py-2 w-full ${
                locError ? "ring-1 ring-red-400 border border-red-400" : ""
              } bg-gray-200`}
            >
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (locError && e.target.value.trim()) setLocError(null);
                }}
                className="bg-transparent w-full focus:outline-none text-gray-600"
              />
            </div>
            {locError && (
              <p className="text-red-600 text-sm mt-1">{locError}</p>
            )}
          </div>
        ) : (
          <>
